...
export const getQuizzesByCursoIdActivo = async (cursoId: number, token: string) => {
  if (!token) throw new Error('Token no proporcionado');
  const response = await axios.get<Quiz[]>(`${API_URL}/quizzes/activos-programados/${cursoId}`, {
    headers: {
      Authorization: `Bearer ${token}`
    }
  });
